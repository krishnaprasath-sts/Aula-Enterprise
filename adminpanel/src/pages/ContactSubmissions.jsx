...
import { 
  ...
  X, Trash2, Eye, MessageSquare, Clock, Filter, Sparkles, Building, Download
} from 'lucide-react';
...
const ContactSubmissions = () => {
  ...
  const loadSubmissions = async () => {
    setIsLoading(true);
    try {
      const data = await contactSubmissionsApi.getAll();
  ...
  useEffect(() => {
    localStorage.removeItem('aula_contact_submissions_list');
  }, []);
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    try {
      await contactSubmissionsApi.updateStatus(id, newStatus);
      const updated = contactSubmissions.map(e => e.id === id ? { ...e, status: newStatus } : e);
      setContactSubmissions(updated);
      if (selectedSubmission && selectedSubmission.id === id) {
        setSelectedSubmission({ ...selectedSubmission, status: newStatus });
      }
      showToast(`Submission #${id} marked as ${newStatus}!`);
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (submissionToDelete) {
      try {
        await contactSubmissionsApi.delete(submissionToDelete.id);
        setContactSubmissions(contactSubmissions.filter(e => e.id !== submissionToDelete.id));
  ...
  const countNew = contactSubmissions.filter(e => e.status === 'New').length;
  const countInProgress = contactSubmissions.filter(e => e.status === 'In Progress').length;
  const countResolved = contactSubmissions.filter(e => e.status === 'Resolved').length;

  const filteredSubmissions = contactSubmissions.filter(e => {
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">View and manage messages submitted from the public Contact Us page.</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '18px', marginBottom: '28px' }}>
        <div className="card" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '14px' }}>
          ...
          <div>
            <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Total Messages</div>
            <div style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--text-primary)' }}>{contactSubmissions.length}</div>
          </div>
        </div>
      ...
      <div className="toolbar">
        ...
        <div className="toolbar-right">
          <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)', fontWeight: 600 }}>
            Showing {filteredSubmissions.length} of {contactSubmissions.length} records
          </span>
        </div>
      </div>
